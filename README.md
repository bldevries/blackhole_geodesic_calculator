...
|:--:| 
| *Figure 5: a two dimension example of how rays are traced by the curvedpy python package. 17 rays all start at x = -15 $R_{\text{Schwarzschild}}$, indicated by a red dot. The black arrow shows the initial direction of the ray. In red you see the calculated ray path. The black disk at the origin represents the blackhole's Schwarzschild radius, also known as the event horizon. The Schwarzschild radius is given by $\frac{2 G M}{c^2}$ and is thus determined by the mass of the blackhole. *|
