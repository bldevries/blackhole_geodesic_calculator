...
class RelativisticRenderEngine(bpy.types.RenderEngine):
    ...
    # #########################################################################
        ...
        #
        #########################
        self.pkl_file = depsgraph.scene.pkl_file
    ...
    # ############################################################################################################################
        ...
        buf = self.ray_trace(depsgraph, self.res_x, self.res_y, 1, buf=[], samples=self.samples)  
        buf.shape = -1,4  
    ...
    # ############################################################################################################################
        ...
        cam = RelativisticCamera()
                           ...
                           #  field_of_view=[0.6, 0.6],\
                           # verbose=True, \
                           # a = 0.9,\
                           # camera_location=np.array([0.00001,-10,10]),\
                           # )
                           #camera_rotation_euler=rot)#np.array([[1,0,0], [0,1,0], [0,0,1]]))#verbose=False)
        #cam.run(verbose=True, verbose_lvl = 100)
        cam.load(self.pkl_file)
        #cam.load("/Users/vries001/Dropbox/0_DATA_BEN/PHYSICS/PROJECTS/curvedpy/pre_run_cameras/mp/res_124x124_fov-x_0.3_fov-y_0.3_a_0.0_M_0.5_xyz0_0.0001_0.0_30.0_rot_TEST_angle_TEST_max_step_0.1.pkl")
        #cam.load("/Users/vries001/Dropbox/0_DATA_BEN/PHYSICS/PROJECTS/curvedpy/pre_run_cameras/res_1024x1024_fov_0.6_a_0.9_xyz0_0.0001_0.0_30_rot_x_angle_0.0.pkl")
        #cam.load("/Users/vries001/Dropbox/0_DATA_BEN/PHYSICS/PROJECTS/curvedpy/pre_run_cameras/res_124x124_fov_0.6_a_0.0_xyz0_0.0001_0.0_30_rot_x_angle_0.0.pkl")
        #cam.load("/Users/vries001/Dropbox/0_DATA_BEN/PHYSICS/PROJECTS/curvedpy/pre_run_cameras/res_124x124_fov_0.6_a_0.0_xyz0_0.0001_-10_10_rot_x_angle_45.pkl")
        #cam.load("/Users/vries001/Dropbox/0_DATA_BEN/PHYSICS/PROJECTS/curvedpy/pre_run_cameras/res_124x124_fov_0.6_a_0.9_xyz0_0.0001_-10_10_rot_x_angle_45.pkl")
        #cam.load("/Users/vries001/Dropbox/0_DATA_BEN/PHYSICS/PROJECTS/curvedpy/pre_run_cameras/res_124x124_fov_0.6_a_0.0_xyz0_0.0001_-10_10_rot_x_angle_45.pkl")
        #print(cam.results)

        for iy, ix in np.ndindex((height, width)):
            if cam.ray_blackhole_hit[iy, ix] == 1:
                sbuf[iy, ix, 0:4] = np.array([0,0,0, 1])
            else:
                sbuf[iy, ix, 0:3] = self.background_hit(direction=cam.ray_end[iy, ix, 3:6])
                sbuf[iy, ix, 3] = 1
                #np.array([1,1,1, 1])


        # for res in results: # Or something like this
